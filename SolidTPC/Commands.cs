...
namespace SolidTPC {
    internal class Commands {
        ...
        public static bool ContainsSubCommand(string key) {
            if (DB.ContainsKey(Parser.MainCommand)) {
                int len = DB[Parser.MainCommand].Length;
                for (int i = 0; i < len; i++) {
                    var a = DB[Parser.MainCommand][i];
                    if (a.word != null && a.word.Contains(key)) {
                        Parser.SubCommandIndex = i;
                        return true;
                    }
                }
            }
        ...
        public static bool ContainsArgument(string key) {
            var a = DB[Parser.MainCommand][Parser.SubCommandIndex].arguments;
            for (int i = 0; i < a.Length; i++) {
                var c = a[i];
                if (c.word != null && c.word.Contains(key)) {
                    Parser.ArgumentIndex = i;
                    return true;
                }
        ...
        public static Argument[] GetArgumentList() {
            return DB[Parser.MainCommand][Parser.SubCommandIndex].arguments;
        }
        ...
        public static bool HasArgument(string word) {
            ...
            //} 
            //return false;
            var a = DB[Parser.MainCommand][Parser.SubCommandIndex].arguments[Parser.ArgumentIndex].types;
            if (a != null) {
                return a.Length > 0;
            }
            return false;            
