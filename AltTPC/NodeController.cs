...
namespace AltTPC {
    internal class NodeController {
        ...
        /**
            ...
            渡したノードを解決する
            ノード中の子も再帰で解決する
            PENDING中のノードがある場合にtrueを返す
            </summary> 
        */
        public static void ResolveNode(Node node) {
        ...
        /**
            ...
            まだ判別がされていない要素である場合は待機リストに追加する
            </summary>
        */
        public static void CheckNodeClose(Node node, char? endToken) {
            ...
            if (node.parent is null) {
                if (node.type == NodeType.BLOCK && node.word.Length == 0) {
            ...
            switch (node.type) {
                ...
                case NodeType.OPERATOR_PERIOD:
                    ...
                    if (node.child[0].returnedType == NodeType.COMMAND_MAIN && node.child[1].returnedType == NodeType.COMMAND_SUB) {
        ...
        public static void SetNodeType(Node node) {
            ...
            if (w == '@') {  // コマンド・CODE型
                ...
                if (Commands.DB.ContainsKey(wl)) {  // メインコマンド
                    ...
                    Interpreter.MainCommand = node.word.ToLower();
